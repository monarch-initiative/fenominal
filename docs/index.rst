...
Fenominal is a Java 16 application for text-mining
`Human Phenotype Ontology (HPO) <http://www.human-phenotype-ontology.org>`_ terms from text. Fenominal is
a multimodule project with a ``core`` module with the text-mining logic, a ``cli`` module for use from the
command line, and a ``gui`` module to provide a Graphical User Interface.
