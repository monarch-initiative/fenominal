...
//                            new MappedSentencePart(chunk, opt.get().getHpoId());
    ...
    private List<DetailedMinedTerm> getBestCandidates(List<SimpleToken> nonStopWords ,
                                                      Map<Integer, List<DetailedMinedTerm>> candidates) {
        // arrange hits according to number of matches tokens
        Map<Integer, List<DetailedMinedTerm>> wordCountToSentencePartListMap = new HashMap<>();
        for (List<DetailedMinedTerm> sentencePartList : candidates.values()) {
            for (DetailedMinedTerm msp : sentencePartList) {
